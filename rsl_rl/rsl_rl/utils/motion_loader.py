...
class AMPLoader:
    EXPECTED_FRAME_TYPE = "amp_expert"
    JOINT_POS_SIZE = 20
    ...
    def __init__(
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file) as f:
                ...
                if "motion_visualization/" in normalized_motion_file:
                    raise ValueError(
                        f"AMPLoader expected expert motion data, got visualization file: {motion_file}"
                    )
                frame_type = motion_json.get("FrameType")
                if frame_type not in (None, self.EXPECTED_FRAME_TYPE):
                    raise ValueError(
                        f"AMPLoader expected FrameType '{self.EXPECTED_FRAME_TYPE}', got '{frame_type}' in {motion_file}"
                    )
                motion_data = np.array(motion_json["Frames"])
                if motion_data.ndim != 2 or motion_data.shape[1] != AMPLoader.END_POS_END_IDX:
                    raise ValueError(
                        f"AMPLoader expected frame shape [N, {AMPLoader.END_POS_END_IDX}], got {motion_data.shape} in {motion_file}"
                    )
