...
class AMPLoaderDisplay:
    EXPECTED_FRAME_TYPE = "visualization"
    JOINT_POS_SIZE = 26
    ...
    def __init__(
        ...
        motion_files=glob.glob("datasets/motion_visualization/*"),
    ):
        ...
        for i, motion_file in enumerate(motion_files):
            ...
            with open(motion_file) as f:
                ...
                normalized_motion_file = motion_file.replace("\\", "/")
                if "motion_amp_expert/" in normalized_motion_file:
                    raise ValueError(
                        f"AMPLoaderDisplay expected visualization motion data, got expert file: {motion_file}"
                    )
                frame_type = motion_json.get("FrameType")
                if frame_type not in (None, self.EXPECTED_FRAME_TYPE):
                    raise ValueError(
                        f"AMPLoaderDisplay expected FrameType '{self.EXPECTED_FRAME_TYPE}', got '{frame_type}' in {motion_file}"
                    )
                motion_data = np.array(motion_json["Frames"])
                if motion_data.ndim != 2 or motion_data.shape[1] != AMPLoaderDisplay.JOINT_VEL_END_IDX:
                    raise ValueError(
                        f"AMPLoaderDisplay expected frame shape [N, {AMPLoaderDisplay.JOINT_VEL_END_IDX}], got {motion_data.shape} in {motion_file}"
                    )
    ...
    def get_frame_at_time_batch(self, traj_idxs, times):
        ...
        max_idx = n.astype(np.int64) - 1
        idx_low = np.minimum(np.floor(p * n).astype(np.int64), max_idx)
        idx_high = np.minimum(np.ceil(p * n).astype(np.int64), max_idx)
